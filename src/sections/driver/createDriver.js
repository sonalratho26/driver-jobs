...
export const CreateDriver = () => {
  ...
  useEffect(() => {
    if (driverData?.statusCode === "200") {
      toast.success(driverData?.message);
      setTimeout(() => {
        router.push('/drivers');
      }, 2000);
     
    } else if (driverData?.status === "failed") {
  ...
  const handleSubmit = () => {
    ...
    formData.append("job_id", values.jobs);
    formData.append("confirmPassword", values.confirmPassword);
    formData.append("image_src", img);
    formData.append("address", values.address);
    if (values.name !== undefined && values.email != undefined && values.role_id !== undefined && values.password !== undefined && values.mobile !== undefined
      && values.jobs !== undefined && values.confirmPassword !== undefined && img !== undefined && values.address !== undefined) {
      dispatch(createDriverActions(formData))
    }
  };
    ...
    >
      ...
      >
              ...
              >
                ...
                >
                 <option
                  ...
                  {jobData.length > 0 && jobData?.map((data, id) => (
                    <option
                    value={data.job_id} key={id}
                    >
                     {data.name}
                    </option>
                  ))}
        ...
        <CardActions sx={{ justifyContent: 'flex-end' }}>
          <Button variant="contained" onClick={ () => {handleSubmit();reviewsHandler()}}>
            Create
          </Button>
        </CardActions>
      </Card>
